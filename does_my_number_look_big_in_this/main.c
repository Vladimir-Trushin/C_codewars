/*
DESCRIPTION:
A Narcissistic Number (or Armstrong Number) is a 
positive number which is the sum of its own digits, 
each raised to the power of the number of digits 
in a given base. In this Kata, we will restrict 
ourselves to decimal (base 10).

For example, take 153 (3 digits), which is narcissistic:

    1^3 + 5^3 + 3^3 = 1 + 125 + 27 = 153

and 1652 (4 digits), which isn't:

    1^4 + 6^4 + 5^4 + 2^4 = 1 + 1296 + 625 + 16 = 1938

The Challenge:

Your code must return true or false (not 'true' and 'false') 
depending upon whether the given number is a Narcissistic number in base 10.

This may be True and False in your language, e.g. PHP.

Error checking for text strings or other invalid inputs 
is not required, only valid positive non-zero integers 
will be passed into the function.
*/


#include <stdbool.h>
#include <math.h>
#include <stddef.h>
#include <stdio.h>
#include <math.h>


size_t count_digit(int num);
bool narcissistic(int num);

int main()
{
    printf("%d %s\n", 153, narcissistic(153) ? "true" : "false");   // true
    printf("%d %s\n", 1652, narcissistic(1652) ? "true" : "false"); // false
    printf("%d %s\n", 2, narcissistic(2) ? "true" : "false");       // true
    printf("%d %s\n", 72, narcissistic(72) ? "true" : "false");     // false
    printf("%d %s\n", 21, narcissistic(21) ? "true" : "false");     // false


    return 0;
}


//-------------------------------------
size_t count_digit(int num)
{
  size_t count = 0;
  
  while (num)
  {
    num /= 10;
    count++;
  }
  
  return count;
}

//-------------------------------------
bool narcissistic(int num)
{
  int sum = 0;
  int t_num = num;
  
  size_t digits = count_digit(num);
  
  for (size_t i = 0; i < digits; i++)
  {
    sum += (int)pow(num % 10, digits); 
    
    num /= 10;
  }
    
  return sum == t_num;
}